...
buttons = [{"name": "textEdit", "text": None}, {"name": "btnPress1", "text": "Convert to Download link"}, {
    "name": "btnPress3", "text": "Clear Field Contents"}, {"name": "btnPress2", "text": "Copy to Clipboard"}]
...
class LinkConverter(QWidget):
    def __init__(self, parent=None):
        ...
        for button in buttons:
            self[button["name"]] = QTextEdit(
            ) if not button["text"] else QPushButton(button["text"])
            layout.addWidget(self[button["name"]])
